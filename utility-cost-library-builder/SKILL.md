...
revision_reason: "enums.yaml gap fixes: EQP→EQS (db-verified), BUS/CAB sections added, PTX added to MEQ"
related_skills:
...
authority:
  canonical:
    ...
    - 18 CFR Part 101                  # FERC Uniform System of Accounts (electric)
...
# Utility Cost Library Builder v3.2 — Electric T&D Matrix Cost Model

> Version 3.2.0 — L7 work package enumeration complete for T.L.CON and D.L.CON. Read §17 (Resolved Architectural Decisions) and §18 (Changelog) to understand what changed from v2 and why.

---

## 1. Session Mission

Build an audit-ready Electric Utility (Transmission and Distribution) cost library using a normalized matrix accounting model. Every cost dollar is stamped with **four orthogonal coordinates** — physical asset (WBS-A), labor pool (WBS-B), permit constraints, and FERC USoA account — joined through the Construction Unit (CU) layer. The output is a single ten-tab Excel workbook in which:
...
| P8 | **7-level WBS depth** — the WBS hierarchy is L1–L7 (PRGR→FNAR→FCTY→PHSE→DISC→DLVR→WKPC). Assumptions, inclusions, exclusions live in metadata columns on the L7 row, never as an 8th level. | A_RULE_03; G1 |
...
Given a `wbs_id` like `E.T.S.P04.CON.BGR.FDN`, determine the FERC account deterministically. This is **the** authoritative routing procedure and replaces all in-skill FERC-mapping prose tables from v2.
...
1. Parse wbs_id:
   - branch_context = L2 + L3   (e.g., T_SUB, T_LIN, D_SUB, D_LIN)
   - L5_code        = parts[4] if depth >= 5 else None
   - L6_code        = parts[5] if depth >= 6 else None
   - leaf_code      = parts[-1]
2. Search WBS_FERC_Linkage in order:
   (a) L6_SPECIFIC + branch_context + applicability=PRIMARY
   (b) L5_WILDCARD + branch_context + applicability=PRIMARY
   (c) L5_WILDCARD + branch_context=ALL + applicability=PRIMARY
3. If a match in (a), (b), or (c): RETURN that account_number.
4. If no match: emit G12 (FERC routing missing) error; HOLD pending user input.
5. CONDITIONAL entries are surfaced as alternatives but do NOT become the PRIMARY return value.
```
...
```
E.T.S.P04.CON  ┐                        E.T.S.ACQ  ┐
E.T.S.ACQ      ┘  (peers under E.T.S)    ├── LND   │  (Acquisitions)
E.T.L.P04.CON  ┐                         ├── PER   │
E.T.L.ACQ      ┘  (peers under E.T.L)    │   ├── PRM │  (permit fees by type)
...
### ADR-08: Two-view accounting (estimating + capitalization)

- **Decision**: The workbook supports both an estimating view (soft costs visible as WBS-A line items in the ACQ branch) and a capitalization view (soft costs roll into physical plant accounts via WBS_FERC_Linkage at booking).
- **Rationale**: This is the canonical pattern observed across every industry framework (ISO-NE PP-4 Att. D mandates 10 estimating categories; NYISO Manual 23 mandates similar breakdown; MISO MTEP25 uses 8; AACE RP 96R-18 organizes Class 5→1 estimates with this structure). Estimating teams need the soft-cost visibility; capitalization teams need the plant-account rollup. The same data serves both.
- **Source**: ISO-NE PP-4 Attachment D, NYISO OATT §30.8, MISO MTEP25 §3, AACE 96R-18 §5.
- **Enforcement**: P7, ACQ branch design (§12.3).

### ADR-09: RCL = Reclamation (SUB); CLR = Clearing (LIN)

- **Decision**: The same 3-letter code `RCL` cannot mean different things in S vs. L contexts. S.GCO retains `RCL` for "Reclamation" (post-construction site restoration); L.GCO uses `CLR` for "Clearing" (vegetation/tree removal pre-construction).
...
- **Rationale**: The temp-vs-permanent distinction was carried by the parent L5 (`GCO.SEC` is temporary by definition because it's under General Conditions; `S.P04.CON.SEC` L5 is permanent infrastructure). The element-name variants added noise without information.
- **Source**: User direction.
- **Enforcement**: `enums.wbs_l6_electric` element_name field.

### ADR-11: STC (Site Conditions) added to T-LIN

- **Decision**: T.L.P04.CON gets an `STC` L5 mirroring D.L.P04.CON.STC (children: GRD, PAV, SRF, UTC). S sections (T and D) do not get STC.
...
- **Source**: User direction.
- **Enforcement**: `enums.wbs_l5_electric` × `branch_context.L`; Registry rows 188–192.
...
### v3.2.1 (2026-05-28) — enums.yaml substation gap fixes

**Changes:**
- Fixed `EQP` → `EQS` (Equipment Support) in `wbs_l7_electric` STR section — `EQP` was a doc error; `EQS` is the canonical code in UtilixV2 `tblNode`
- Added `BUS` section to `wbs_l7_electric`: RIG (Rigid Bus), FLX (Flexible Bus), STB (Bus Support), BCN (Bus Connection); JMP and INS noted as shared same-code nodes
- Added `CAB` section to `wbs_l7_electric`: CTL (Control Cable), LVC (LV Power Cable), TRY (Cable Tray), LBL (Labels); GND and TRM noted as shared same-code nodes
- Added `PTX` (VT — voltage/potential transformers) to MEQ section
- Bumped `schema_version` to 3.2.1; `SKILL.md` header version updated to match

**Database verified:** All codes confirmed present in UtilixV2 `tblNode` before writing.

### v3.2.0 (2026-05-28) — L7 work package enumeration

**Changes:**
- Removed TEN, ENV, COM disciplines from L5 — only CON (Construction) remains active for P04
- Added complete L7 work package enumeration under T.L.P04.CON and D.L.P04.CON
- WIR deliverable: added SPL (Splice), JMP (Jumper), DMP (Damper), ARM (Armor Rod), SPC (Spacer), SHW (Shield Wire)
- BGR deliverable: added ANC (Anchor)
- GCO deliverable: added TMP (Temp Security)
- SCT deliverable: updated to SWT, RCL, FUS, SRA
- MEQ deliverable: added TRF (Transformer), REG (Regulator), CAP (Capacitor) for D.L only
- enums.yaml updated to reflect all new L7 codes

**Database alignment:** UtilixV2 tblNode and tblEdge now contain complete T.L and D.L Construction WBS hierarchies down to L7.

### v3.1.0 (2026-05-28) — Code updates

**Changes:**
- L3 Facility Type codes updated: `LIN` → `L` (Line), `SUB` → `S` (Substation)
- L4 Phase codes updated to numbered scheme: `PLN` → `P00`, `DEV` → `P01`, `ENG` → `P02`, `PCR` → `P03`, `PEX` → `P04`, `CLO` → `P05`
- Example paths updated: `E.T.SUB.PEX.CON.BGR.FDN` → `E.T.S.P04.CON.BGR.FDN`
- Branch context enum updated: `T_SUB` → `T_S`, `T_LIN` → `T_L`, `D_SUB` → `D_S`, `D_LIN` → `D_L`
- 7-level WBS hierarchy formalized (PRGR→FNAR→FCTY→PHSE→DISC→DLVR→WKPC)

**No breaking schema changes** — this is a code rename only. Column structures and join-tab relationships unchanged.
...
- v2 wbs_id format `WBS-edx-NNNNNN` surrogate → replaced by dot-path composite key `E.T.S.P04.CON.BGR.FDN`.
...
| OVH | Overhead | L.CON only |
| WIR | Wire (overhead conductors) | L.CON only |
| SCT | Sectionalizing | L.CON only |
| FIB | Fiber / Comms | S.CON, L.CON |
| GCO | General Conditions | All CON branches |
| MEQ | Major Equipment | S.CON; D.L.CON only |
| STC | Site Conditions | L.CON only (T and D) |
| BUS | Buswork | S.CON only |
| CAB | Control / LV Cable | S.CON only |
| CHS | Control House | S.CON only |
| PRO | Protection & Control | S.CON only |
| SEC | Security (permanent) | S.CON only |
| SSV | Station Service | S.CON only |
| STR | Above-Grade Structures | S.CON only |
| SWK | Sitework / Yard Prep | S.CON only |
...
### 19.3 FERC USoA accounts in scope (28 accounts)

| Account | Title | Branch |
|---|---|---|
| 183 | Preliminary Survey and Investigation Charges | BOTH |
| 301 | Organization | INTANGIBLE |
| 302 | Franchises and Consents | INTANGIBLE |
| 303 | Miscellaneous Intangible Plant | INTANGIBLE |
| 350 | Land and Land Rights (Transmission) | T |
| 351 | [Reserved] | T |
| 352 | Structures and Improvements (Transmission) | T |
| 353 | Station Equipment (Transmission) | T |
| 354 | Towers and Fixtures (Transmission) | T |
| 355 | Poles and Fixtures (Transmission) | T |
| 356 | Overhead Conductors and Devices (Transmission) | T |
| 357 | Underground Conduit (Transmission) | T |
| 358 | Underground Conductors and Devices (Transmission) | T |
| 359 | Roads and Trails (Transmission) | T |
| 360 | Land and Land Rights (Distribution) | D |
| 361 | Structures and Improvements (Distribution) | D |
| 362 | Station Equipment (Distribution) | D |
| 363 | Storage Battery Equipment (Distribution) | D |
| 364 | Poles, Towers and Fixtures (Distribution) | D |
| 365 | Overhead Conductors and Devices (Distribution) | D |
| 366 | Underground Conduit (Distribution) | D |
| 367 | Underground Conductors and Devices (Distribution) | D |
| 368 | Line Transformers (Distribution) | D |
| 369 | Services (Distribution) | D |
| 370 | Meters (Distribution) | D |
| 371 | Installations on Customer Premises | D |
| 373 | Street Lighting and Signal Systems | D |
| 374 | Asset Retirement Costs for Distribution Plant | D |
